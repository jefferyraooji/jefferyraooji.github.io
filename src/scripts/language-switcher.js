...
function updatePageContent(lang) {
    ...
    const translations = {
        'en': {
            ...
            'travelDescription': 'Recording my travel footprints and beautiful memories',
            ...
            // Profile
            'profileBio': 'Jincheng(Jeffery) Rao is a master student at Duke University. He is a Machine Learning summer intern at Forest Vision.',
            
            // Contact
            'contact': 'Contact',
            'contactMe': 'Contact Me',
            'contactTitle': 'Get In Touch',
            'contactDescription': 'Feel free to reach out if you have any questions or would like to collaborate!',
        ...
        'zh_cn': {
            ...
            // Profile
            'profileBio': 'Jincheng(Jeffery) Rao是杜克大学的研究生。他是Forest Vision的机器学习暑期实习生。',
...
function updateTravelPage(t) {
    ...
    if (title && (title.textContent.includes('Travel Footprint') || title.textContent.includes('旅游足迹'))) {
    ...
    if (desc && (desc.textContent.includes('Recording my travel') || desc.textContent.includes('记录我的旅行'))) {
    ...
    stats.forEach(stat => {
        if (stat.textContent.includes('destinations') || stat.textContent.includes('目的地')) {
            stat.textContent = stat.textContent.replace(/destinations|目的地/g, t.destinations);
        }
        if (stat.textContent.includes('trips') || stat.textContent.includes('旅行')) {
            stat.textContent = stat.textContent.replace(/trips|旅行/g, t.trips);
        }
    });
    
    // 更新旅行卡片中的城市名称
    const cityNames = ['Liverpool', 'Manchester', 'Boston', 'Seattle', 'Hawaii', 'Washington D.C.', 'Miami', 'New York', 'Zurich', 'Geneva', 'Vatican', 'Rome', 'Venice', 'Milan', 'London', 'Edinburgh'];
    ...
    // 更新旅行卡片中的日期
    const dates = ['September 19, 2022', 'April 6, 2023', 'July 4, 2025', 'May 13, 2025', 'May 6, 2025', 'December 31, 2024', 'December 18, 2024', 'October 12, 2024', 'June 3, 2024', 'June 1, 2024', 'March 20, 2024', 'March 18, 2024', 'March 16, 2024', 'March 14, 2024', 'July 23, 2023', 'March 27, 2023'];
    dates.forEach(date => {
        const elements = document.querySelectorAll('*');
        elements.forEach(el => {
    ...
    // 更新旅行卡片中的位置
    const locations = ['Liverpool, England', 'Manchester, England', 'Boston, USA', 'Seattle, USA', 'Hawaii, USA', 'Washington D.C., USA', 'Miami, USA', 'New York, USA', 'Zurich, Switzerland', 'Geneva, Switzerland', 'Vatican City', 'Rome, Italy', 'Venice, Italy', 'Milan, Italy', 'London, UK', 'Edinburgh, Scotland'];
...
function updateFooter(t) {
    ...
    if (footer) {
        ...
        if (text.includes('All Rights Reserved') || text.includes('版权所有')) {
            footer.innerHTML = footer.innerHTML.replace(/All Rights Reserved|版权所有/g, t['All Rights Reserved']);
        }
        if (text.includes('RSS')) {
            footer.innerHTML = footer.innerHTML.replace(/RSS/g, t.RSS);
        }
        if (text.includes('Sitemap') || text.includes('网站地图')) {
            footer.innerHTML = footer.innerHTML.replace(/Sitemap|网站地图/g, t.Sitemap);
        }
        if (text.includes('Powered by') || text.includes('技术支持')) {
            footer.innerHTML = footer.innerHTML.replace(/Powered by|技术支持/g, t['Powered by']);
        }
    }
}
...
function updateAboutPage(t) {
    ...
    if (aboutTitle && (aboutTitle.textContent.includes('About') || aboutTitle.textContent.includes('关于'))) {
        aboutTitle.textContent = t.About;
    }
    
    const aboutDesc = document.querySelector('p');
    if (aboutDesc && (aboutDesc.textContent.includes('This is Jeffery') || aboutDesc.textContent.includes('这是Jeffery'))) {
        aboutDesc.textContent = t['This is Jeffery\'s personal website built with Astro.'];
    }
}
...
function updateProfile(t) {
    ...
    bioElements.forEach(el => {
        ...
        if (text.includes('Jincheng(Jeffery) Rao is a master student') || text.includes('Jincheng(Jeffery) Rao是杜克大学的研究生')) {
...
function updateContactPage(t) {
    ...
    if (contactTitle && (contactTitle.textContent.includes('Get In Touch') || contactTitle.textContent.includes('与我联系'))) {
        contactTitle.textContent = t.contactTitle;
    }
    
    // 更新Contact页面描述
    const contactDesc = document.querySelector('p');
    if (contactDesc && (contactDesc.textContent.includes('Feel free to reach out') || contactDesc.textContent.includes('如果您有任何问题'))) {
